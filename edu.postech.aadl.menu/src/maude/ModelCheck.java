...
public class ModelCheck extends Maude {
	...
	class MaudeRunner extends Thread {
		...
		public String getSolutionString(String rm) {
			if (rm.indexOf("result Bool") != -1) {
				return rm.substring(rm.indexOf("result Bool"));
			} else if (rm.indexOf("result ModelCheckResult") != -1) {
				return rm.substring(rm.indexOf("result ModelCheckResult"));
			} else {
